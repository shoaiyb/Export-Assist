...
if ( !current_user_can('export') ){
	wp_die( __('Sorry, you are not allowed to export the content of this site.', 'export-assist') );
}
$e = new WP_Error();
$updated = array();
$entry = '';
...
if(isset($_GET["ew2bc_download"]) && ctype_xdigit($_GET["ew2bc_download"]) && is_admin() && isset($_GET["ew2bc_nonce"])){
	...
	if( $_GET["type"] == "post" ){
		...
		if (in_array(false, array_map('is_numeric', $category)) || empty($category)) {
			$e->add( 'error', __('Selected categories are invalid.', 'export-assist') );
		}else{
		...
		$e->add( 'error', __('Selected type is invalid.', 'export-assist') );
	}
	...
	if( $_GET["status"] == "any" ){
		...
		$e->add( 'error', __('Selected status is invalid.', 'export-assist') );
	}
			...
			foreach( $export_posts as $post ){
				...
				if($post->post_type == "post"){
					...
					if( strlen( $labelcount ) > 199 ){
						$tagline = '';
						$catline = '';
						$e->add( 'error', __('Post ', 'export-assist').'ID: '.$thisid.' - '.__('The combined length of all the labels must be under 200 characters. Please reduce categories or tags for this post. (now:', 'export-assist').strlen( $labelcount ).')' );
					}
...
<ns0:feed xmlns:ns0="http://www.w3.org/2005/Atom"><ns0:link href="http://www.blogger.com/" rel="self" type="application/atom+xml" /><ns0:title>MovableType blog</ns0:title><ns0:updated>2019-01-01T00:00:00Z</ns0:updated><ns0:generator>Blogger</ns0:generator><ns0:link href="http://www.blogger.com/" rel="alternate" type="text/html" />'.$entry.'</ns0:feed>';
			...
			if(empty($e->get_error_messages('error'))){
				$updated[] = __('XML file successfully exported.', 'export-assist');
			}
		}else{
			//can't find any post error
			$e->add( 'error', __('Cannot find any data to export.', 'export-assist') );
		}
	}
